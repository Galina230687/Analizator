...
public class Main {
    ...
    public static Thread getThread(BlockingQueue<String> queue, char letter) {
        return new Thread(() ->
        {
            int max = finalMaxCharCount(queue, letter);
            System.out.println(letter + " символов -> " + max);
        });
    }

    public static int finalMaxCharCount(BlockingQueue<String> queue, char letter) {
        int count = 0;
        int max = 0;
        String text;
        try {
